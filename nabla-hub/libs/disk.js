...
function checkDiskSpace(directoryPath) {
  ...
  function checkUnix(unixDirectoryPath) {
    if (!normalize(unixDirectoryPath).startsWith(sep)) {
      return new Promise((resolve, reject) => {
        reject(new InvalidPathError(`The following path is invalid (should start by ${sep}): ${unixDirectoryPath}`));
      });
    }

    const pathToCheck = getFirstExistingParentPath(unixDirectoryPath);
